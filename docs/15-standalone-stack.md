...
| `AGENTKIT_REGISTRY_ALWAYS_PULL` | `true` forces a pull on every `EnsurePresent` instead of reusing a locally-present image. Required for the `:dev`-tag dev flow (`./stack start … registry`), where the tag is stable but its contents drift on every republish. Forwarded by `docker-compose.yml` since 2026-08-13 — before that, setting it in `.env` silently did nothing |
