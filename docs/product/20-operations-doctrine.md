...
| OM-10 | **Attribute signals by actor; never count them project-wide.** Where a legitimate worker moves the same counter a signal reads, a project-wide total reports the innocent org as guilty — and the guilty one as ordinary. | evidenced-offline | SC3: the critic writes prompts and trips the frozen boundary every round, so compliance is counted only when the *dispatcher* is the actor (smoke: freeze_refused 7 vs directed 1) |
