...
| **architect-archivist@v1 — the recommended shape** | `features/architect-archivist.stack.spec.ts`, needs `--mock-script` **and** `--idle-timeout 1m` | the seed's org chart, and then the loop it describes actually running: you **chat as the architect** (`POST /agent/session {"worker":…}`) and its prompt reaches the model; a typo'd worker is a **404**, not a bare session; the architect **builds the roster it proposed** with the ordinary management tools, logged with itself as actor; the frozen archivist **refuses** it and the attempt lands as a `worker.freeze_refused` *event* while the config log records no change; **a conversation that goes quiet wakes the archivist**, which writes memory under the project's policy — and the archivist's own completion produces **no second delivery**, the self-delivery guard holding on an *unfiltered* subscription; and **a dispatched job's completion wakes it too**, which is exactly what dropping the old `interactive=true` filter bought. Until 2026-08-08 none of this could run at all: the session route dropped the `worker` the console had been sending, so a chat carried no identity, so it never emitted `worker.finished` |
