...
| `embedding.json` | The embeddable-Orange e2e (`features/embedding.stack.spec.ts`, design 2026-08-06 T17): two firings of ONE session-mode schedule into one long-lived session, keyed on `[T17-HYP]` in the schedule input. Two rules, split with `absent: "[T17-RUN1-DONE]"` — and that split, rather than a turn offset, is the point. **The turn index resets on every user message in this rig**, because the in-image agent calls the SDK with `persistSession: false` and no resume, folding prior turns into the system prompt as prose (`sandbox/src/harness/claude-agent-sdk.ts:186-199`). So turn indices are per-FIRING, and a second message to the same session is told from the first only by a marker the first one left in its final reply. Firing 1 writes `/workspace/hypothesis.md` with `mcp__ui__write_file` (which auto-registers it as an artifact) and a >500-byte memory with `mcp__core__memory_create`; the spec then snapshots the container away; firing 2 `cat`s the file back out of the restored container, reads the memory with `mcp__core__memory_current`, and rewrites the file with round-2 content |
