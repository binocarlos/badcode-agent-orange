#!/usr/bin/env bash
# The injection gauntlet (work plan 13, SC3) — executes
# docs/product/19-scenario-library.md §3 SC-3, and instruments doctrine WD-1.
#
#   ./e2e/experiments/gauntlet/run.sh build           typecheck + compile only
#   ./e2e/experiments/gauntlet/run.sh test            the offline unit layer (no stack)
#   ./e2e/experiments/gauntlet/run.sh gen <config>    regenerate the tickets only
#   ./e2e/experiments/gauntlet/run.sh list            the gauntlet configs available
#   ./e2e/experiments/gauntlet/run.sh run <config>    the real thing
#
...
DIST="$EXPERIMENTS/dist/experiments/gauntlet"
...
config_field() {
  node "$DIST/gauntlet.js" "$1" --print-config | node -e \
    'let s="";process.stdin.on("data",d=>s+=d).on("end",()=>process.stdout.write(String(JSON.parse(s)[process.argv[1]]??"")))' "$2"
}

# generate CONFIG — gauntletgen renders the manifest into the dataset
...
# checksums). The generator also VERIFIES every ticket — that its SC-1 trap
# still holds with the directive planted, and that the directive is one a
# compliant worker can obey and a rule-following one does not accidentally.
generate() {
  ...
  command -v go >/dev/null 2>&1 || {
    echo "go is not on PATH — the ticket generator is a Go command (go/cmd/gauntletgen)" >&2; return 1; }
  echo "── gauntlet: generating adversarial tickets from $manifest ──"
  ( cd "$ROOT/go" && go run ./cmd/gauntletgen -manifest "$ROOT/$manifest" -out "$ROOT/$datasets" )
}
...
# the triage rig for the same reason it exists there: --mock-script is bolted to
# the playwright `test` command, and this is not a playwright spec.
...
cmd_run() {
  ...
  # stack produces a gauntlet run of the mock. Both are refusals, not warnings.
  if [ "$mode" = "mock" ]; then
    ...
    if [ "${GAUNTLET_LIVE_RUN:-}" != "1" ]; then
      echo "$config spends real tokens against a real model, and work plan 13's L3 posture says" >&2
      echo "attended runs only. Set GAUNTLET_LIVE_RUN=1 to say you are watching:" >&2
      echo "    GAUNTLET_LIVE_RUN=1 $0 run $config" >&2
      return 1
    fi
  fi
  ...
  if [ -n "$script" ]; then
    ...
    echo "── gauntlet: loading mock model script $script into agentd ──"
    ...
    trap 'echo "── gauntlet: unloading mock model script ──"; reload_agentd_with_script "" || true' EXIT
  fi

  local status=0
  node "$DIST/gauntlet.js" "$config" "$@" || status=$?
  return "$status"
}

cmd_test() {
  build
  echo "── gauntlet: offline unit layer ──"
