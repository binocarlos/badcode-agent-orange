#!/usr/bin/env bash
# The calibration rig (work plan 13, L3R) — executes
# docs/product/14-calibration-runbook.md.
#
#   ./e2e/experiments/calibration/run.sh build           typecheck + compile only
#   ./e2e/experiments/calibration/run.sh test            the offline unit layer (no stack)
#   ./e2e/experiments/calibration/run.sh gen <config>    regenerate the datasets only
#   ./e2e/experiments/calibration/run.sh list            the calibration configs available
#   ./e2e/experiments/calibration/run.sh run <config>    the real thing
#
...
#
# The live calibration, which costs tokens and is ATTENDED (work plan 13, L3):
#
...
# experiments/tsconfig.json and sweeps this directory with it. One tsconfig, one
# dist/ — the C1↔B1 collision in docs/product/13's log is what that rule is for.
set -euo pipefail
...
DIST="$EXPERIMENTS/dist/experiments/calibration"
...
config_field() {
  node "$DIST/calibrate.js" "$1" --print-config | node -e \
    'let s="";process.stdin.on("data",d=>s+=d).on("end",()=>process.stdout.write(String(JSON.parse(s)[process.argv[1]]??"")))' "$2"
}

# generate CONFIG — hypolabgen renders the manifest into the dataset directory.
# Always regenerated: the datasets are gitignored, deterministic, and a stale
# directory is refused by the loader anyway (truths.json carries checksums).
generate() {
  ...
  command -v go >/dev/null 2>&1 || {
    echo "go is not on PATH — the dataset generator is a Go command (go/cmd/hypolabgen)" >&2; return 1; }
  echo "── calibration: generating datasets from $manifest ──"
  ( cd "$ROOT/go" && go run ./cmd/hypolabgen -manifest "$ROOT/$manifest" -out "$ROOT/$datasets" )
}
...
# the C1 rig for the same reason it exists there: --mock-script is bolted to the
# playwright `test` command, and this is not a playwright spec.
...
cmd_run() {
  ...
  # stack produces a calibration of the mock. Both are refusals, not warnings.
  if [ "$mode" = "mock" ]; then
    ...
    if [ "${CALIBRATION_LIVE_RUN:-}" != "1" ]; then
      echo "$config spends real tokens against a real model, and work plan 13's L3 says" >&2
      echo "attended runs only. Set CALIBRATION_LIVE_RUN=1 to say you are watching:" >&2
      echo "    CALIBRATION_LIVE_RUN=1 $0 run $config" >&2
      return 1
    fi
  fi
  ...
  if [ -n "$script" ]; then
    ...
    echo "── calibration: loading mock model script $script into agentd ──"
    ...
    trap 'echo "── calibration: unloading mock model script ──"; reload_agentd_with_script "" || true' EXIT
  fi

  local status=0
  node "$DIST/calibrate.js" "$config" "$@" || status=$?
  return "$status"
}

cmd_test() {
  build
  echo "── calibration: offline unit layer ──"
