...
Use the pre-built component library at `./src/lib/components/` to build dashboards quickly. Instead of writing 200+ lines of D3/Chart.js code, import components and configure them in ~30 lines.
...
This is what your entire `main.js` should look like:
...
// 2. Get data
const awareness = getRecords('brand_awareness')
const consideration = getRecords('consideration')
const nps = getRecords('nps_scores')
...
```javascript
const records = getRecords('dataset_name')
...
After writing main.js:
```bash
cd /workspace/app && npx vite build
```
Then register:
```
register_artifact(file_path="app/dist/index.html", artifact_type="webapp", label="Dashboard Title")
```
