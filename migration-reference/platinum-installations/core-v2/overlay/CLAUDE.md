...
For JavaScript webapps: verify each dataset with `node /workspace/lib/viz/inspect-data.js <name>` before writing `webapp/app.js`. Confirm it has the rows/columns you expect.
...
- **Web app**: there is no build step. After editing `webapp/app.js` (and `index.html`/`style.css` as needed), register the webapp by pointing at its entry HTML:
  ```
  register_artifact(file_path="webapp/index.html", artifact_type="webapp", label="...", description="...")
  ```
  This captures the **entire `webapp/` directory** (`app.js`, `style.css`, `lib/`, and any images you added), so the whole app is extracted to cloud storage. There is no auto-detection for webapps -- you must register explicitly. You do not need to separately register `app.js`/`style.css`; they ride along inside the webapp directory.
...
The session's customer, job, and user email are set via environment variables (`SESSION_CUSTOMER`, `SESSION_JOB`, `SESSION_USER_EMAIL`). All `pt` commands and `render_table`/`render_chart` tools inherit customer/job automatically. **Never guess or fabricate customer or job names.** If you need a different job, use `pt jobs` to list available jobs.

The user's personal TOC folder is `Tables/User/<SESSION_USER_EMAIL>/`. Use this path to:
...
Every webapp/dashboard you ship must include a Data Reference Document at `webapp/data-reference.md` showing the spec, filter, weight, case filter, and base size for every dataset, plus chart-to-dataset mapping. After registering the webapp, `ask_user` to verify the figures before treating the dashboard as final. Load the **data-visualization** skill for the template.

### 14. Logos and Images in Webapps
Never draw client logos as SVG approximations — always use the provided image file. Copy any referenced image into the `webapp/` directory (`cp /workspace/uploads/X /workspace/webapp/X`) and reference it with a relative path (`./logo.png`, never `/logo.png`), then verify with `ls`. Everything under `webapp/` is captured when you register. Load the **data-visualization** skill for the full checklist.
...
## Webapp Workspace (no build — plain HTML)

Webapps here are **plain HTML with no build step**. There is **no Vite, no npm, no
`node_modules`, no `dist/`**. You write the page and register it directly. This is
fast and has no failure modes — do not introduce a bundler or run `npm`/`vite`.

The workspace ships a ready starter at **`/workspace/webapp/`**:

```
webapp/index.html   <- import map (loads d3/chart.js/three/plot from a CDN) + links style.css + app.js
webapp/app.js       <- YOUR dashboard code goes here
webapp/style.css    <- curated dark-theme design system (edit, don't replace)
webapp/lib/data-loader.js   <- async loader for /workspace/data/*.json
webapp/lib/platinum-data.js <- low-level PlatinumData transforms
webapp/lib/components/      <- reusable, theme-aware chart + layout components
```

**Libraries load from a CDN via the import map in `index.html`** — so you still
`import * as d3 from 'd3'` (or `chart.js/auto`, `three`, `@observablehq/plot`) exactly
as before, with no install. Need another library? Add one line to the import map in
`index.html`, then import it by name. Do not run `npm install`.
...
3. Edit /workspace/webapp/app.js using the data-loader (async — await it):
     import { getRecords, getMatrix } from './lib/data-loader.js'
     import { barChart } from './lib/components/index.js'
     const records = await getRecords('awareness')   // [{row, col, value}], 0-100
     barChart(document.getElementById('app'), { records, title: 'Awareness' })
4. screenshot_url(url="/workspace/webapp/index.html")   // verify it renders
5. register_artifact(file_path="webapp/index.html", artifact_type="webapp")
```

Step 5 registers the whole `webapp/` directory (the captured webapp includes
`app.js`, `style.css`, and `lib/`). Pass `datasetName` to `render_table` so
`getRecords('awareness')` works directly. There is **no `vite build`** step.

### Data-loader API (async)

Import from `./lib/data-loader.js` in `app.js`. **These are async — `await` them**
(the data is fetched at runtime, not bundled):

| Function | Parameter | Returns |
|----------|-----------|---------|
| `await getRecords(name, metric?)` | dataset name | `[{row, col, value}]` flat records (D3, Plot) |
| `await getMatrix(name, metric?)` | dataset name | `{rows, columns, values}` (Chart.js) |
| `await getDatasetMeta(name)` | dataset name | `{top, side, filter, weight, name}` |
| `await loadDataset(name)` | dataset name | raw PlatinumData object |
| `await listDatasets()` | none | `string[]` (best-effort; pass explicit names instead) |

Values are already 0-100 percentages. Never multiply by 100. Never hardcode labels.
Metric options: `'colpc'` (default), `'rowpc'`, `'freq'`. You always know your
dataset names because you set them via `datasetName` — pass them explicitly.

### Reusable components

`./lib/components/index.js` exports theme-aware builders: `barChart`, `lineChart`,
`pieChart`, `radarChart`, `heatmap`, `dataTable`, `hero`, `kpiCards`, `chartSection`,
`sectionNav`, `footer`, `applyTheme`. Each takes `(containerEl, config)`. Prefer
these over hand-rolling charts — focus on choosing the right component and shaping
the data, not on building chart internals.

**Use the existing `style.css`** — dark-theme dashboard with CSS variables, grid
layouts, cards, KPI widgets. Edit, don't replace.

### Debugging data (Node.js)

The browser data-loader uses `fetch`, which does not run in Node. To inspect data
from the shell, use the inspect tool:
...
Place any referenced image inside `webapp/` and reference it with a relative path
(`./logo.png`). Everything under `webapp/` is captured when you register the webapp.

**Screenshotting web apps:** `screenshot_url(url="/workspace/webapp/index.html")`. It
serves `/workspace` over HTTP so relative `../data` paths resolve. Never start servers
manually (`vite preview`, `npx serve`, etc.) — they hang the session.
