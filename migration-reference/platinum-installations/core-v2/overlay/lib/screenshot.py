...
def screenshot(url, output_path, width=1280, height=800, wait_ms=3000):
    ...
    if not url.startswith(('http://', 'https://', 'file://')):
        ...
        if not os.path.isfile(abs_path):
            ...
            sys.exit(1)
        # Root the server at /workspace (when the target is inside it) so that
        # relative paths like ../data/x.json and ../../data/x.json resolve exactly
        # as they do when the webapp is served back to the user. Otherwise serve
        # the file's own directory.
        workspace = '/workspace'
        if abs_path == workspace or abs_path.startswith(workspace + os.sep):
            serve_dir = workspace
            rel_url = os.path.relpath(abs_path, workspace)
        else:
            serve_dir = os.path.dirname(abs_path)
            rel_url = os.path.basename(abs_path)
