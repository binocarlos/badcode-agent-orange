...
func TestResolveLaunchImage_CustomImageMaterialized(t *testing.T) {
	...
	ref, err := r.resolveLaunchImage(context.Background(), "", "img-1", "a@acme.com", "acme", nil)
	if err != nil {
...
func TestResolveLaunchImage_CustomImageNotVisible_FallsBackToBase(t *testing.T) {
	...
	ref, err := r.resolveLaunchImage(context.Background(), "", "img-missing", "a@acme.com", "acme", nil)
	if err != nil {
		t.Fatalf("resolve: %v", err)
	}
...
func TestResolveLaunchImage_ExplicitImageStillWinsOverCustom(t *testing.T) {
	...
	ref, err := r.resolveLaunchImage(context.Background(), "explicit:tag", "img-1", "a@acme.com", "acme", nil)
	if err != nil {
		t.Fatalf("resolve: %v", err)
	}
